...
async function sendNotifications(title, body) {
  try {
    webpush.setVapidDetails('mailto:admin@thebrief.app', process.env.VAPID_PUBLIC_KEY, process.env.VAPID_PRIVATE_KEY);
    const indexRaw = await kv.get('sub:index');
    const index = indexRaw ? (typeof indexRaw === 'string' ? JSON.parse(indexRaw) : indexRaw) : [];
    const payload = JSON.stringify({ title, body });
    await Promise.allSettled(index.map(async (key) => {
